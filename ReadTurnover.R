...
DionTurnoverFN = "~/Google Drive/CoChIPAnalysis/NucTurnover.rdata"

if( file.exists(DionTurnoverFN)) {
  DionList = readRDS(DionTurnoverFN)
  NucTurnoverRate = DionList$Rate
  NucTurnoverZ = DionList$Z
} else {
  ...
  for( l in c( 1, 5,  10, 25, 50, 75,  100) ) {
    ...
    if( length(Probes) > 0 ) {
      rs <- GRanges( seqnames = Chr[Probes],
                     ...
                     seqinfo = seqinfo(NucRegions) # Seqinfo(genome="sacCer3")
      )
  ...
  saveRDS(list(Rate = NucTurnoverRate, Z = NucTurnoverZ), DionTurnoverFN )
} 
